...
function biCubicSpline(xData, yData, zData, N, lambda)
    ...
    d2z1dxdy_1= [1/(deltaX[i]*deltaY[j])*(
    ...
    d2z1dy2_1 = [1/(deltaY[j]^2)*((-6+6*xTilde+6*yTilde)*zData[i,j]
    ...
    d2z1dxdy_2 = [1/(deltaY[j]*+deltaX[i])*(
    ...
    d2z1dy2_2 = [1/(deltaY[j]^2)*((-6+6*yTilde+6*(1-xTilde))*zData[i+1,j]
    ...
    d2z1dxdy_3 = [1/(deltaX[i]*deltaY[j])*(
    ...
    d2z1dy2_3 = [1/(deltaY[j]^2)*((-6+6*( 1-xTilde )+6*( 1-yTilde ))*zData[i+1,j+1]
    ...
    d2z1dxdy_4 = [1/(deltaX[i]*deltaY[j])*(
    ...
    d2z1dy2_4 = [1/(deltaX[i]^2)*((-6+6*( 1-yTilde )+6*xTilde)*zData[i,j+1]
    ...
    @objective(model, Min, sum( sum( 1/(N^2)*(
            sum( sum( abs_d2zdx2_1[i,j, k ] + 2*abs_d2z1dxdy_1[i,j, l ] + abs_d2z1dy2_1[i,j, k, l ]  
                    + abs_d2zdx2_2[i,j, l ] + 2*abs_d2z1dxdy_2[i,j, k ] + abs_d2z1dy2_2[i,j, k, l ] 
                    + abs_d2zdx2_3[i,j, k ] + 2*abs_d2z1dxdy_3[i,j, l ] + abs_d2z1dy2_3[i,j, k, l ] 
                    + abs_d2zdx2_4[i,j, l ] + 2*abs_d2z1dxdy_4[i,j, k ] + abs_d2z1dy2_4[i,j, k, l ] 
    ...
    @constraint(model,pos_d2z1dxdy_1[i in 1:I-1, j in 1:J-1, l in 1:(2*N) ], abs_d2z1dxdy_1[i, j, l] >=  d2z1dxdy_1[i, j, l] )
    @constraint(model,neg_d2z1dxdy_1[i in 1:I-1, j in 1:J-1, l in 1:(2*N) ], abs_d2z1dxdy_1[i, j, l] >= -d2z1dxdy_1[i, j, l])

    @constraint(model,pos_d2z1dy2_1[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_1[i, j, k, l] >=  d2z1dy2_1[i, j, k, l] )
    @constraint(model,neg_d2z1dy2_1[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_1[i, j, k, l] >= -d2z1dy2_1[i, j, k, l])
    ...
    @constraint(model,pos_d2z1dxdy_2[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_2[i, j, k] >=  d2z1dxdy_2[i, j, k] )
    @constraint(model,neg_d2z1dxdy_2[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_2[i, j, k] >= -d2z1dxdy_2[i, j, k])

    @constraint(model,pos_d2z1dy2_2[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_2[i, j, k, l] >=  d2z1dy2_2[i, j, k, l] )
    @constraint(model,neg_d2z1dy2_2[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_2[i, j, k, l] >= -d2z1dy2_2[i, j, k, l])
    ...
    @constraint(model,pos_d2z1dxdy_3[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_3[i, j, k] >=  d2z1dxdy_3[i, j, k] )
    @constraint(model,neg_d2z1dxdy_3[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_3[i, j, k] >= -d2z1dxdy_3[i, j, k])

    @constraint(model,pos_d2z1dy2_3[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_3[i, j, k, l] >=  d2z1dy2_3[i, j, k, l] )
    @constraint(model,neg_d2z1dy2_3[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_3[i, j, k, l] >= -d2z1dy2_3[i, j, k, l])
    ...
    @constraint(model,pos_d2z1dxdy_4[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_4[i, j, k] >=  d2z1dxdy_4[i, j, k] )
    @constraint(model,neg_d2z1dxdy_4[i in 1:I-1, j in 1:J-1, k in 1:(2*N) ], abs_d2z1dxdy_4[i, j, k] >= -d2z1dxdy_4[i, j, k])

    @constraint(model,pos_d2z1dy2_4[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_4[i, j, k, l] >=  d2z1dy2_4[i, j, k, l] )
    @constraint(model,neg_d2z1dy2_4[i in 1:I-1, j in 1:J-1, k in 1:(2*N), l in 1:(2*N) ], abs_d2z1dy2_4[i, j, k, l] >= -d2z1dy2_4[i, j, k, l])
